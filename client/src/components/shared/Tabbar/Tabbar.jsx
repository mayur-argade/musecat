...
const Tabbar = () => {
    ...
    return (
        <section className="shadow-lg">
            <nav
                className={`hidden md:flex flex-wrap justify-center py-1 space-x-6`}
            >
                {loading ? (
                        ...
                        <div className="relative z-30">
                            {categories.data && categories.data.length > 7 && (
                                <div
                                    ...
                                    className="mt-1 relative flex align-middle items-center hover:font-bold"
                                >
                                    <BlurIn
                                        word="More"
                                    ...
                                    />
                                    <img
                                        className="cursor-pointer dark:hidden flex h-5 mt-0.5"
                                        src="/images/icons/add.svg"
                                        alt="Add Icon"
                                    />
                                    <img
                                        className="cursor-pointer dark:flex hidden h-2 ml-2"
                            ...
                            {categories.data &&
                                ...
                                showMoreCategories && (
                                    <div
                                        ...
                                        className="h-auto w-40 p-3 absolute right-0 bg-white dark:bg-[#454545] drop-shadow-md rounded"
                                    >
                                        {categories.data
                                            .slice(7)
                                            .map((category, index) => (
                                                <>
                                                    {category.name !==
                                                        "Ladies Night" && (
                                                        <div
                                                            className="flex hover:bg-slate-50 dark:hover:bg-slate-500 flex-col py-1"
                                                            key={index}
                                                        >
                                                            <CategoryLink
                                                                category={
...
const CategoryLink = ({ category, handleCategoryChange, disableAnimation }) => {
    ...
    return (
        <div className="dark:bg-transparent" key={category.id}>
            {category.subCategories && category.subCategories.length > 0 ? (
                                    ...
                                    <div className="w-full py-1">
                                        <li key={index}>
                                            <span
                                                onClick={() =>
                                                    navigate(
                                                        `/category/${category.categoryURL}?subcategory=${subcategory.name}`,
                                                    )
                                                }
                                                onMouseEnter={() => {
                                                    if (
                                                ...
                                                className={`hover:font-bold cursor-pointer ml-0 block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-base font-medium`}
                                                aria-current="page"
                                            >
                                                {subcategory.name}
                                            </span>
                                                    ...
                                                    >
                                                        {[
                                                            ...
                                                            <li
                                                                onClick={() =>
                                                                    navigate(
                                                                        `/category/${category.categoryURL}?subcategory=${subcategory.name}&day=${day}`,
                                                                    )
                                                                }
                                                            ...
                                                            >
                                                                <a
                                                                    href="#"
                                                                    className={`block text-sm py-4 my-2  pl-3 pr-4 md:p-0 hover:font-bold md:dark:font-bold`}
                                                                    aria-current="page"
                                                                >
                                                                    {day} Dinner
                                                                </a>
                ...
                >
                    <div
                        className={`hover:font-bold ml-0 px-1 py-1 text-base duration-500 ease-out cursor-pointer ${window.location.pathname === `/category/${category.categoryURL}` ? "font-bold" : "font-normal"}`}
                    >
                        {disableAnimation ? (
                            <span className="ml-0">{category.name}</span>
                        ) : (
                            <BlurIn word={category.name} className="ml-0" />
                        )}
                    </div>
                </Link>
            )}
