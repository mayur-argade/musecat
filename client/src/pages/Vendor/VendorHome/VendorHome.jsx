...
const VendorHome = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const [homeData, eventData] = await Promise.all([
                    VendorHomeApi(),
                    VendorUnverifiedEvents()
                ]);

                setReponse(homeData.data); // Access data within each response object
                setUnverifiedEvents(eventData.data);

            } catch (error) {
                console.error("Error fetching data:", error); // Handle error more appropriately
            } finally {
                setIsLoading(false);
            }
        };
        ...
        // Cleanup function for overflow style
        return () => {
                                        ...
                                        response.data.ongoingEvents.length != 0 && (
                                            <div className='mt-5 flex flex-col'>
                                                ...
                                                <div className="grid grid-flow-row gap:8 md:mx-3 md:gap-3 text-neutral-600 grid-cols-2 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4">
                                                    {response.data.ongoingEvents.length != 0 ?
                                                        response.data.ongoingEvents.slice(0, 3).map((event) => (
                                                ...
                                                <div className="grid grid-flow-row gap:8 md:mx-3 md:gap-3 text-neutral-600 grid-cols-2 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4">
                                                    {
                                                        eventsLoading
                                                            ...
                                                            :
                                                            unverifiedEvents.data.slice(0, 3).map((offer) => (
