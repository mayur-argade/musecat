...
import { VendorVerify } from '../../../http/index'
import { useParams } from 'react-router-dom'
import { Link } from 'react-router-dom'

const VerifyVendorAccount = () => {
    let { token } = useParams();
    ...
    useEffect(() => {
        const verifyUser = async () => {
            try {
                setLoading(true)
                const res = await VendorVerify(token)
                console.log(res)
                setMessage(res.data)
                setLoading(false)
            } catch (error) {
        ...
        return (
            <div className="bg-green-50 min-h-screen flex items-center justify-center">
                <div className="bg-white p-16 rounded-lg shadow-lg">
                    ...
                    <Link className='flex justify-center mt-5' to='/vendor/login'>
...
export default VerifyVendorAccount